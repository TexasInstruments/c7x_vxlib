...
def gen_test_case_header_file(testCase):
    ...
    inp = np.random.uniform(low=minValInp, high=maxValInp, size=(height, width)).astype(
        VXLIB_utils.resolveNumpyDType(dTypeIn)
    )
    out = np.array(
        list(
            map(lambda x: [trueVal if (y > thresholdVal) else falseVal for y in x], inp)
        )
    )
