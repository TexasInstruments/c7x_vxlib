                                  ...
                                  const VXLIB_dilate_InitArgs *pKerInitArgs)
{
   ...
   // Using vector type directly without unused elemType
...
VXLIB_STATUS VXLIB_dilate_exec_ci(VXLIB_kernelHandle handle, void *restrict pIn, void *restrict pOut)
{
   ...
   for (int32_t wCounter = 0; wCounter < static_cast<int32_t>(wBlocks); wCounter++) {

      for (int32_t hCounter = 0; hCounter < static_cast<int32_t>(hBlocks); hCounter++) {

         vecType vMaxA = (vecType) 0;
         vecType vMaxB = (vecType) 0;

         for (int32_t fCounter = 0; fCounter < static_cast<int32_t>(filterWidth * filterHeight); fCounter++) {
